...
def compare_diff(branch, resultA=LAST_RESULT_FILE, resultB=RESULT_FILE):
    with open(resultA, 'r') as last, open(resultB, 'r') as curr:
        ...
        with open(DIFF_FILE, 'w') as f:
            ...
            if len(diff_set) == 0:
                f.write(" Current total : " + str(len(curr_set)) +
                ...
                f.write(" Current total : " + str(len(curr_set)) +
                        ...
                        "\n Change test cases : " + str(len(diff_set)))

        TEMP_DIFF = TEMP_FILE + str(
...
def send_mail(file_name):
    ...
    resp = os.popen(
        'git log --pretty=tformat:%h-%cn-%ce-%an-%ae -1').readline()
    ...
    PASSED = subprocess.run("grep PASSED " + RESULT_FILE + " | wc -l",
                            shell=True,
                            stdout=subprocess.PIPE,
                            encoding="utf-8").stdout.strip()
    FAILED = subprocess.run("grep FAILED " + RESULT_FILE + " | wc -l",
                            shell=True,
                            stdout=subprocess.PIPE,
                            encoding="utf-8").stdout.strip()
    PANICKED = subprocess.run("grep PANICKED " + RESULT_FILE + " | wc -l",
                              shell=True,
                              stdout=subprocess.PIPE,
                              encoding="utf-8").stdout.strip()
    TIMEOUT = subprocess.run("grep TIMEOUT " + RESULT_FILE + " | wc -l",
                             shell=True,
                             stdout=subprocess.PIPE,
                             encoding="utf-8").stdout.strip()
    TOTAL = subprocess.run("grep result " + RESULT_FILE + " | wc -l",
                           shell=True,
                           stdout=subprocess.PIPE,
                           encoding="utf-8").stdout.strip()
    ...
    message.attach(
        MIMEText(
            ...
            format("不通过率 : " + str((
                (float(FAILED) + float(PANICKED) + float(TIMEOUT)) /
                float(TOTAL)) * 100) + "%") + '\n' +
            format("超时率 : " + str((float(TIMEOUT) / float(TOTAL)) * 100) + "%")
            + '\n' + ("崩溃率 : " + str(
                (float(PANICKED) / float(TOTAL)) * 100) + "%") + '\n' +
...
if os.path.exists(LAST_RESULT_FILE):
    ...
    subprocess.run('cp ' + RESULT_FILE + " " + BASE + "/warehouse/" + "zCore" +
                   "_realm/" + user + "/result/" + branch + "/test-result" +
