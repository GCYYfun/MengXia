...
args = [
    "-smp", "cores=4", "-drive", "if=pflash,format=raw,readonly,file=" + OVMF,
    "-drive", "format=raw,file=fat:rw:" + ESP, "-serial", "mon:stdio", "-m",
    "4G", "-device", "isa-debug-exit", "-drive", "format=qcow2,file=" + IMG +
    ",media=disk,cache=writeback,id=sfsimg,if=none", "-device",
    "ahci,id=ahci0", "-device", "ide-drive,drive=sfsimg,bus=ahci0.0", "-accel",
    "kvm", "-cpu", "host", "-nographic"
]
...
def main():
    ...
    with open(LogFile, "w", encoding="utf-8", errors="ignore") as fd:
        ...
        for line in lines:
            ...
            index = child.expect([
                "successed", "failed", pexpect.EOF, pexpect.TIMEOUT,
                "/libc-test #", "Hangup", "=== BEGIN rCore stack trace ==="
            ])
            if index == 0:
